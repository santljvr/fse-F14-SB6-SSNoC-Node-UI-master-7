...
function User(user_name, password, salt, userStatusCode, locationDesc, lastPostTime){ //modified by feng
  this.local = {
	user_id : '',
    name : user_name,
    password : password,
    salt : salt,  // added by santosh
...
User.getUser = function(user_name, callback) {
  request(rest_api.get_user + user_name, {json:true}, function(err, res, body) {
    ...
    if (res.statusCode === 200) {
      var user = new User(body.userName, body.password, body.salt, body.status, body.locationDesc, body.lastPostTime); //modified by feng
      user.user_id = body.user_id; //user id add in this restful api by qihao, so this place needs to be modified
      callback(null, user);
  ...
  request(rest_api.get_all_users, {json:true}, function(err, res, body) {
    ...
    if (res.statusCode === 200) {
      var users = body.map(function(item, idx, arr){
        ...
        var tmpUser =  new User(item.userName, item.password, item.salt, item.status, item.locationDesc, item.lastPostTime);//modified by feng
